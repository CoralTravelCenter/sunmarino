...
- есть минимальная JS-логика:
  ...
  - если `top-offset` не задан, верхний offset вычисляется при инициализации через `matchMedia`: mobile `81px`, tablet `65px`, desktop `16px`
...
- `top-offset` задает явный override; без него offset определяется через `matchMedia` при инициализации
